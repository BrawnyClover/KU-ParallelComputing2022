...
void MasterDynamicProcess(int process_cnt, int block_width);
...
void SlaveProcess(int rank, int block_width)
{
	...
	do {
		...
		printf("Slave %d : calculating color from row %d to %d\n", rank, rows[0], rows[block_width-1]);
...
void MasterDynamicProcess(int process_cnt, int block_width)
{
	...
	for (int prc = 0; prc < process_cnt - 1; prc++) {
		...
		printf("Master : Send Rows, from %d to %d\n", rows[0], rows[block_width-1]);
		MPI_Send(rows, block_width, MPI_INT, prc + 1, STATUS_CALC, MPI_COMM_WORLD);
		task_running++;
	}
	printf("Master : generated task, remain : %d\n", task_running);
	...
	while (task_running > 0) {
		...
		if (row_cnt < DISPLAY_HEIGHT) {
			...
			printf("Master : terminate task, remain : %d\n", task_running);
		}
		for (int i = 0; i < block_width; i++) {
	...
	for (int i = 0; i < DISPLAY_WIDTH; i++) {
		for (int j = 0; j < DISPLAY_HEIGHT+1; j++) {
			fprintf(output, "%d ", result[i*(DISPLAY_HEIGHT+1) + j]);
		}
		fprintf(output, "\n");
	}*/
	generateBitmapImage();
...
int main(int argc, char* argv[])
{
	...
	int block_width = DISPLAY_WIDTH / N_rectanle;

	if (me == 0) { // Master
		printf("Master : MasterProcessStart\n");
		MasterDynamicProcess(np, block_width);
		printf("Master : Process done\n\n");
	}
	else { // Slave
		printf("Slave %d : SlaveProcessStart\n", me);
